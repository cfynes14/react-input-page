...
const InputPage = () => {
  ...
  const [focusedElement, setFocusedElement] = useState<FocusElements>(
    FocusElements.UserName,
  );
  ...
  const handleFocus = (key: DirectionKeys) => {
    switch (key) {
      case DirectionKeys.Up:
        const shouldFocusOnUserName = () =>
          focusedElement !== FocusElements.SubmitButton;
  ...
  const handleInputChange = (button: string) => {
    const fieldStateUpdater =
      focusedElement === FocusElements.UserName ? setUsername : setUserPassword;
    const fieldToUpdate =
      focusedElement === FocusElements.UserName ? userName : userPassword;
    if (button === "{bksp}") {
      fieldStateUpdater(fieldToUpdate.substring(0, fieldToUpdate.length - 1));
      return;
    }
    ...
    <div>
      ...
      <div className="input-page-container">
        ...
        <div className="input-field-container">
          <InputField
            value={userName}
            isFocused={focusedElement === FocusElements.UserName}
            type="username"
            key={1}
          />
          <div className="password-field-container">
            <InputField
              value={userPassword}
              hiddenValue={hiddenPassword}
              passwordHidden={isPasswordHidden}
              isFocused={focusedElement === FocusElements.Password}
              type="password"
              key={2}
            />
            <ShowHidePasswordbutton
              isFocused={focusedElement === FocusElements.ShowHidePassword}
